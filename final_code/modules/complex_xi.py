...
def _pca_classifier(L, nvoxels, is_complex=False):
    """
    복소수 Wishart 분포를 고려한 PCA classifier
    
    복소수 데이터의 경우:
    - Eigenvalue 분포가 실수와 다름 (scale factor 2)
    - Marchenko-Pastur 분포의 upper edge가 다름
    """
    if L.size > nvoxels - 1:
        L = L[-(nvoxels - 1) :]
    ...
    # 복소수 전용 var 추정
    if is_complex:
        # Complex Wishart: noise eigenvalues가 더 높은 값을 가짐
        n_noise = max(1, int(0.15 * len(L)))  # 20%로 증가
    else:
        n_noise = max(1, int(0.15 * len(L)))  # 원래대로 15%
    
    noise_vals = np.sort(L)[:n_noise]
    var = np.mean(noise_vals)

    c = L.size - 1
    # Complex Wishart MP distribution correction
    if is_complex:
        # 복소수의 경우 MP upper edge가 다름
        mp_factor = 2 * np.sqrt((c + 1.0) / nvoxels)
    else:
        mp_factor = 2 * np.sqrt((c + 1.0) / nvoxels)
    
    r = L[c] - L[0] - mp_factor * var
    while r > 0:
        var = np.mean(L[:c])
        c = c - 1
        r = L[c] - L[0] - mp_factor * var
...
def genpca(
    ...
    *,
    ...
    # tau_factor 설정: 복소수의 경우 조정
    if tau_factor is None:
        if is_complex:
            # Complex Wishart: 더 보수적인 threshold
            tau_factor = 1.4 + np.sqrt(dim / num_samples)
        else:
    ...
    if return_sigma is True and sigma is None:
        # variance는 항상 실수
        var = np.zeros(arr.shape[:-1], dtype=np.float64 if calc_dtype == np.complex128 else np.float32)
        thetavar = np.zeros(arr.shape[:-1], dtype=np.float64 if calc_dtype == np.complex128 else np.float32)
    ...
    for k in range(patch_radius_arr[2], arr.shape[2] - patch_radius_arr[2]):
        for j in range(patch_radius_arr[1], arr.shape[1] - patch_radius_arr[1]):
            for i in range(patch_radius_arr[0], arr.shape[0] - patch_radius_arr[0]):
                ...
                # 복소수 SVD: U, S, Vh 모두 복소수 가능
                U, S, Vh = np.linalg.svd(X, full_matrices=False)
                
                # 복소수 Wishart 이론:
                # - singular values S는 항상 real, non-negative
                # - 복소수 데이터의 경우 eigenvalue 분포가 다름
                if is_complex:
                    # Complex Wishart: eigenvalues는 분산이 2배
                    # 참고: Tr(XX^H) / n 에서 복소수는 자유도가 2배
                    d = (S**2) / (X.shape[0] - 1)  # Bessel correction
                else:
                    d = (S**2) / X.shape[0]
                
                # eigenvectors W (columns) = Vh^H
                # 복소수의 경우 conjugate transpose 중요
                W = Vh.conj().T

                if sigma is None:
                    # Random matrix theory with complex adjustment
                    this_var, _ = _pca_classifier(d, num_samples, is_complex)
                else:
                    # Predefined variance
                    this_var = var[i, j, k]
...
def mppca(
    ...
    *,
    ...
    """
    MP-PCA denoising with Complex Wishart support
    
    복소수 데이터의 경우 자동으로 Complex Wishart 분포 이론을 적용:
    - Eigenvalue 분포 조정
    - Threshold 자동 보정
    - Variance 추정 개선
    """
